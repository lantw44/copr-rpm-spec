...
%if "%{cross_arch}" == "arm64"
BuildRequires: %{cross_triplet}-libxml2
%endif
BuildRequires: chrpath
Requires:   %{cross_triplet}-glibc
%if "%{cross_arch}" == "arm64"
Requires:   %{cross_triplet}-libxml2
%endif
Provides:   %{cross_triplet}-gcc-stage2 = %{version}
...
%make_build all-{gcc,target-libgcc}
%endif
%if "%{cross_stage}" == "pass2"
    --enable-languages=c \
    --enable-shared \
...
%make_build all-{gcc,target-{libgcc,libatomic,libssp}}
%endif
%if "%{cross_stage}" == "final"
...
%endif
    --with-target-libxml2-include=%{cross_sysroot}/usr/include/libxml2 \
    --with-target-libxml2-lib=%{cross_sysroot}/usr/%{lib_dir_name} \
    --enable-shared \
    --enable-threads=posix \
    --enable-libmulflap \
    --enable-libgomp \
...
%if "%{cross_stage}" == "pass1"
%{__make} DESTDIR=%{buildroot} install-{gcc,target-libgcc}
%endif

%if "%{cross_stage}" == "pass2"
%{__make} DESTDIR=%{buildroot} install-{gcc,target-{libgcc,libatomic,libssp}}
...
%if "%{cross_stage}" == "final"
chrpath -d %{buildroot}%{cross_sysroot}/%{lib_dir_name}/libasan.so
%if "%{cross_arch}" == "arm64"
chrpath -d %{buildroot}%{cross_sysroot}/%{lib_dir_name}/libgcobol.so
chrpath -d %{buildroot}%{cross_sysroot}/%{lib_dir_name}/libhwasan.so
chrpath -d %{buildroot}%{cross_sysroot}/%{lib_dir_name}/liblsan.so
chrpath -d %{buildroot}%{cross_sysroot}/%{lib_dir_name}/libtsan.so
%endif
chrpath -d %{buildroot}%{cross_sysroot}/%{lib_dir_name}/libubsan.so
%endif
...
rm -f %{buildroot}%{_bindir}/%{cross_triplet}-%{cross_triplet}-ga68
...
%{_prefix}/lib/gcc/%{cross_triplet}/%{major_version}/include/stdcountof.h
...
%{cross_sysroot}/%{lib_dir_name}/libgcc_s_asneeded.so
%{cross_sysroot}/%{lib_dir_name}/libatomic.a
%{cross_sysroot}/%{lib_dir_name}/libatomic.so
%{cross_sysroot}/%{lib_dir_name}/libatomic.so.1
%{cross_sysroot}/%{lib_dir_name}/libatomic.so.1.*
%{cross_sysroot}/%{lib_dir_name}/libatomic_asneeded.a
%{cross_sysroot}/%{lib_dir_name}/libatomic_asneeded.so
...
%{cross_sysroot}/%{lib_dir_name}/libga68.a
%{cross_sysroot}/%{lib_dir_name}/libga68.so*
%{cross_sysroot}/%{lib_dir_name}/libga68.spec
%if "%{cross_arch}" == "arm64"
...
%changelog
* Sun May 24 2026 Ting-Wei Lan <lantw44@gmail.com> - 16.1.0-1
- Update to new stable release 16.1.0
- Enable Algol 68 support
- Build libatomic in the second bootstrap stage
